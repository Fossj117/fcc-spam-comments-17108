...
parameters = {
    "api_key": "",
    "q": "\"plan currently under consideration at the FCC to repeal Obama's Title II power grab\"",
...
for offset in range(0, MAX_STEP+1, STEP):
    ...
    fn = "fcc-ecfs-17-108-%s-%s.json" % (offset, offset + parameters['limit'])
    with open(fn, 'w') as f: f.write(resp.content)

# Combine
raw_filings = []
for i, fn in enumerate(glob.glob("*.json")):
    print "Loading %s" % fn
    raw_filings.append(json.load(open(fn)))
